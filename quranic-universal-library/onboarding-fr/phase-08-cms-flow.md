...
> **Ce fichier :** parcourir une ressource représentative à travers le CMS — du brouillon à la ligne Quran publiée jusqu'au téléchargement public.

---

## Ce que nous traçons

**Exemple représentatif :** une **traduction ayah par ayah** (`ResourceContent` avec `sub_type: translation`, `cardinality_type: 1_ayah`).

C'est la boucle éditoriale la plus courante car elle combine :

- Synchronisation source externe (QuranEnc)
...
Les autres types de ressources (tafsir, mushaf, audio) réutilisent la même **forme** avec des modèles et méthodes d'export différents — noté à la fin.
...
**FAIT** — `config.default_namespace = :cms`, donc chaque fichier `ActiveAdmin.register X` devient `/cms/x`.

**FAIT** — Rôles dans `app/models/ability.rb` :
...
Les contributeurs communautaires **ne touchent jamais `/cms`** en général — ils utilisent `/translation_proofreadings` et les admins fusionnent leur travail.

---

## Le hub : `ResourceContent`

`ResourceContent` est le **paquet logique** pour tout jeu de données Quran téléchargeable. Il vit dans la **DB Quran** (`QuranApiRecord`) mais est lié aux lignes catalogue CMS.
...
**Distinction critique (facile à manquer) :**
...
**FAIT** — `ApproveDraftTranslationJob` **n'appelle pas** `refresh_export!`. Les admins doivent rafraîchir les téléchargements séparément (ou exécuter un pipeline d'export complet ailleurs).

---

## Chemin A — Sync QuranEnc (source externe → brouillons)

Utilisé quand `ResourceContent` a `meta_data['source'] == 'quranenc'` ou une `quranenc-key`.
...
**FAIT** — `lib/importer/quran_enc.rb` :
...
**FAIT** — `DraftContent::CheckContentChangesJob` s'exécute chaque semaine (dimanche 06:00, `config/sidekiq_scheduler.yml`) pour détecter les changements de version QuranEnc et enqueue les imports.
...
**FAIT** — Le `Translation.text` publié **n'est pas** modifié. Seule une ligne brouillon est créée.
...
`Draft::Translation#import!` copie `draft_text` → `Translation.text`, gère les notes de bas de page, définit `imported: true`, touche `ResourceContent`.

**FAIT** — Utilise `PaperTrailAttribution` (`attribute_versions_to(user)`) pour le suivi de version sur la ligne publiée.
...
**INFÉRENCE :** Ce sont des **exports opérateur** (email à l'admin, fichiers sous `public/exported_translations` en dev). Utiles pour le débogage, pas le chemin catalogue orienté consommateur.
...
## Champs `meta_data` que vous verrez (traduction)
...
**FAIT** — `ResourceContent` inclut le concern `HasMetaData` pour les helpers `meta_value` / `set_meta_value`.
...
**INFÉRENCE :** La traduction est le meilleur modèle car elle a la boucle brouillon → approbation → export la plus complète. Audio et mushaf sautent ou raccourcissent l'étape brouillon.

---

## Pièges courants (lisez avant votre première session CMS)

1. **Approuver ≠ exporter.** Le texte Quran publié peut être à jour alors que les fichiers `/resources` sont obsolètes jusqu'à ce que quelqu'un clique **Refresh downloads**.

2. **`approved` sur ResourceContent ≠ traductions publiées.** Le booléen est un flag catalogue/visibilité, pas un état de sync du contenu.

3. **L'import en masse désactive PaperTrail temporairement.** Ne vous attendez pas à des lignes de version pour chaque ligne dans une approbation en masse de 6000 ayahs.

4. **`save(validate: false)` est normal.** Les modèles de contenu core sautent souvent les validations ActiveRecord (Phase 3).

5. **Deux systèmes d'export.** Export sidebar (email admin) vs `refresh_export!` (catalogue public). Jobs différents, emplacements de sortie différents.
...
**INFÉRENCE :** Les modérateurs peuvent trier les brouillons mais ont probablement besoin d'un admin pour l'approbation en masse et le rafraîchissement export.
...
| # | Question | Label |
|---|---|---|
| 1 | Y a-t-il un `refresh_export!` automatisé après approbation en masse en production (cron/hook absent du dépôt) ? | **INCONNU** — non trouvé dans le code |
| 2 | Critères exacts pour créer la ligne `DownloadableResource` initiale | **INFÉRENCE** — premier run `Exporter::DownloadableResources` ou création admin manuelle |
| 3 | Si les exports sidebar `ExportTranslationJob` alimentent jamais le catalogue public | **INFÉRENCE :** Non — chemin séparé de `refresh_export!` |
| 4 | Workflow permissions modérateur complet en pratique | **INCONNU** — attribution des rôles est opérationnelle |
...
La Phase 9 trace le **flux runtime** d'édition d'une ressource du point de vue du contributeur (clics navigateur → contrôleur → modèle → ce qui est sauvegardé).

1. Pourquoi les brouillons sont dans la DB CMS mais les traductions dans la DB Quran — qu'est-ce qui casse si vous oubliez ça ?
2. Après approbation en masse de 500 brouillons, quelles deux URLs vérifieriez-vous pour confirmer le succès ?
...
Répondez avec vos questions, ou dites **« proceed »** pour la **Phase 9 — Flux runtime core (édition d'une ressource)**.

---

*Généré pendant l'onboarding contributeur QUL. Phase 8 sur ~24. Investigation en lecture seule — aucun code modifié.*
