# Phase 23 — Carte des surfaces de contribution et préparation

> **Série d'intégration :** plongée progressive pour devenir un contributeur légitime de QUL.  
> **Prérequis :** [Phases 1–22](phase-01-what-is-qul.md)  
> **Ce fichier :** où vous pouvez réellement aider, selon vos compétences — plus une liste de contrôle honnête pour savoir quand vous êtes prêt à contribuer.

---

## Vous êtes ici

Si vous avez lu les Phases 1–22, vous avez un **modèle mental fonctionnel** de QUL :
...
### Profil A — Ingénieur React / TypeScript / Node (vous)
...
| Surface | URL / chemin d'entrée | Zone code | Risque | Commencer ici ? |
|---|---|---|---|---|
...
| **Segment builder (Vue)** | `/surah_audio_files/.../segment_builder` | `app/javascript/segments/` | Moyen–Élevé | Si vous connaissez l'UX audio |
...
**INFÉRENCE :** Votre chemin le plus rapide vers une PR mergée est **docs + tests de services + Stimulus** — domaines qui correspondent à votre stack et ont une CI fine mais des critères de revue clairs.
...
Référence rapide quand vous êtes perdu :

| Si vous travaillez sur… | Relire |
|---|---|
...
## Niveaux de préparation (auto-évaluation honnête)

Cochez chaque case. **Niveau 3 = prêt pour des PR code significatives.** Niveau 4 = prêt pour le travail pipeline données.
...
**Vous pouvez :** Utiliser les données QUL dans vos apps ; signaler des problèmes de données via les templates GitHub.
...
**Vous pouvez :** Explorer en local ; corriger la doc ; ouvrir des issues informées.
...
**Vous pouvez :** PRs docs, tests de services, fixes Stimulus, améliorations search/parser.
...
**Vous pouvez :** Travail approve/import/export avec revue ; édition CMS ciblée.
...
- [ ] Comprend `segment_locked`, règles skip export, anomalies cross-DB (alignement ligne mushaf)
- [ ] Peut ajouter une vérification d'intégrité ou extension job approve en sécurité

**Vous pouvez :** Posséder le cycle de vie complet d'un type de ressource.
...
| Améliorer `project-setup.md` | `app/views/docs/markdown/project-setup.md` | Vous avez eu la douleur setup → corrigez-la |
...
## Ce que vous devriez encore apprendre sur le terrain

Aucune série d'intégration ne remplace ceci — vous l'acquerrez via les PR :
...
## Anti-patterns (ne vous dites pas prêt si…)
...
## Aide-mémoire une page (imprimez ceci)
...
Si vous avez répondu **Quran DB**, **`bin/setup` → charger dump → `bin/dev`**, et **docs ou test service** — vous êtes prêt à contribuer.
...
| Si vous êtes au Niveau… | Faites ceci |
|---|---|
...
| 4+ | Commenter une issue ouverte en proposant d'implémenter avec votre approche |

---

## Intégration terminée

Cette série était une **investigation en lecture seule** sauf si vous avez exécuté des expériences locales vous-même. Vous avez maintenant :
...
- Une **carte** de où vous brancher (cette phase)

On ne vous attend pas pour connaître chaque modèle ou tâche rake. On vous attend pour savoir **où chercher**, **quoi ne pas casser**, et **comment demander**.
...
**INFÉRENCE** — Ne pas bundler 23 fichiers dans une PR fonctionnalité sans lien.
