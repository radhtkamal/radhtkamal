...
> **Ce fichier :** ce qui se passe réellement au runtime quand un contributeur édite une ressource — du clic navigateur à l'écriture en base de données.

---

## Périmètre

Nous traçons la **relecture de traduction ayah** de bout en bout car c'est l'exemple le plus clair du modèle contributeur QUL :

- **Lire** le contenu publié depuis la DB Quran
- **Écrire** les suggestions dans la DB CMS comme brouillons
- **Ne jamais** muter le texte publié dans la session contributeur

Ensuite nous contrastons **l'édition layout mushaf**, qui rompt le modèle brouillon et écrit directement dans la DB Quran.

---

## URLs et routes

**FAIT** — `config/routes.rb` :
...
**FAIT** — Repli `resource_id` par défaut si absent :
...
**INFÉRENCE :** Les presenters sont légers pour cet outil — surtout titres/SEO. Le chargement des données est dans le contrôleur (il y a même un `# TODO: use presenter to load the translation` sur `#show`).
...
**FAIT** — Validations `UserProject` : raison, maîtrise de la langue, motivation, reconnaissance de relecture.
...
**FAIT** — `show` et `index` sont **publics** (lecture traductions publiées). Seuls edit/update nécessitent connexion + approbation projet.
...
**FAIT** — jQuery est encore utilisé dans certains contrôleurs Stimulus (`remote_form_controller.js` utilise `$()`).
...
**INFÉRENCE :** Modèle brouillon = texte haut volume avec revue éditoriale. Écriture directe = données structurelles/layout avec effet immédiat.
...
| Plusieurs lignes brouillon par ayah | **FAIT** — pas d'index unique sur `(resource_content_id, verse_id)` ; chaque soumission crée une **nouvelle** ligne |
...
**INFÉRENCE :** Les strong params empêchent l'assignation en masse de `imported`, `need_review`, etc. — ceux-ci sont définis côté serveur dans `save_suggestions`.
...
**FAIT** — `ApplicationController#set_paper_trail_whodunnit` définit `whodunnit` à `current_user.to_gid` pour les requêtes qui déclenchent PaperTrail.
...
| # | Question | Label |
|---|---|---|
| 1 | Comment les admins choisissent parmi plusieurs lignes brouillon pour le même `verse_id` | **INFÉRENCE** — le job en masse importe tous les `imported: false` ; la dernière écriture par lot gagne par verset |
| 2 | Si les contributeurs sont notifiés quand les brouillons sont approuvés | **INCONNU** — aucun mailer trouvé sur ce chemin |
| 3 | Pourquoi `resource_id` par défaut est codé en dur `131` | **INCONNU** — probablement une traduction anglaise bien connue pour démo |
| 4 | Si `save_suggestions` devrait upsert au lieu d'insérer toujours | **INFÉRENCE** — le comportement actuel permet l'historique d'édition via plusieurs lignes brouillon |
...
L'écart intentionnel entre **suggestion** et **publication** est le design central. Votre travail en tant que développeur d'outils contributeur : ne jamais appeler `translation.update` depuis les contrôleurs contributeur sauf si vous changez délibérément l'architecture.
...
1. Après soumission d'un contributeur, quelle table interrogeriez-vous pour vérifier l'écriture — `translations` ou `draft_translations` ?
...
Répondez avec vos questions, ou dites **« proceed »** pour la **Phase 10 — Pipeline d'import**.

---

*Généré pendant l'onboarding contributeur QUL. Phase 9 sur ~24. Investigation en lecture seule — aucun code modifié.*
