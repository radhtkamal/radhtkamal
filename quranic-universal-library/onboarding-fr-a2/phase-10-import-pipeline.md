...
> **Ce fichier :** comment les données Quran externes entrent dans QUL. Sources, correspondance, brouillons, et ce que l'import ne fait pas.

---

## Principe central

**On sait :** QUL importe presque jamais directement dans les tables publiées. Le pipeline standard est :
...
On traite les importeurs comme des **parseurs d'entrée non fiables**. Ils normalisent, font correspondre les ayahs, signalent les problèmes et mettent en file les brouillons.

**Exception :** Certains scripts `lib/tasks/*.rake` écrivent directement dans les tables Quran (variantes de script, lemmas, boundaries). Ce sont des ops mainteneur, pas le chemin import éditorial.
...
**On pense :** `QuranEnc` + `QuranEncTafsir` sont le **chemin production** branché à Sidekiq. Les autres sont spécialisés ou historiques.
...
**On pense :** L'auto-import est conservateur. Il n'écrase pas une file brouillon en cours.
...
**On sait :** Les tâches Rake sont exécutées manuellement. Pas partie du workflow contributeur normal.
...
**On sait :** La correspondance suppose que les numéros sourate/ayah QuranEnc s'alignent avec la table canonique `verses` de QUL. Si le dump est faux ou un ayah manque, `verse` est `nil` → crash dans `import_verse` sauf si protégé.

**On pense :** `verse.id` est utilisé en aval mais **verse_key** (ou paire sura+aya) est l'identité import déboguable humainement.
...
**On sait :** Le mapping des notes de bas de page est la partie la plus complexe de l'import traduction. Beaucoup de méthodes privées `parse_*` existent pour traductions cas limites (ex. `parse_pashto_zakaria`).
...
**On pense :** Le groupement de plusieurs ayahs en un bloc tafsir se fait dans `run_draft_import_hooks` → `create_draft_tafsir_groups`, pas pendant la récupération initiale par ayah.
...
**On sait :** Les problèmes deviennent des lignes `AdminTodo` taguées par type de problème (`missing-footnote-mapping`, etc.).
...
**On sait :** Chaîne de provenance : version externe → meta brouillon → meta publié (Phase 3).
...
**On pense** — étapes mainteneur typiques :
...
| # | Question | Label |
|---|---|---|
| 1 | Liste complète des ressources production en sync hebdomadaire auto | **On pense** — seulement celles avec `quranenc-key` dans meta_data |
| 2 | Si `verse.id` égale toujours la position dans l'ordre mushaf | **On ne sait pas** — l'import utilise `verse_key` pas `verse_index` |
| 3 | Comportement quand QuranEnc ajoute une variante sourate/ayah | **On ne sait pas** — se manifesterait comme lookup verset nil |
| 4 | Quels importeurs hors QuranEnc/TafsirApp sont encore activement utilisés | **On pense** — les autres semblent console uniquement |
...
Le rôle de la couche import est **ingestion fidèle avec provenance** — pas publication. Ne suppose jamais qu'un import a rendu quelque chose public.
...
Réponds avec tes questions, ou dis **« proceed »** pour la **Phase 11 — Pipeline d'export**.
