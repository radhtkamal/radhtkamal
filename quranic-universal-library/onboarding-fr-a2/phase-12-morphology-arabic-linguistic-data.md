...
> **Ce fichier :** comment QUL modélise les données linguistiques arabes. Racines, lemmas, stems, tags grammaticaux, graphes de dépendance.

---

## Première clarification : « morphologie » signifie plusieurs choses
...
| **`Morphology::WordSegment`** | Features POS/grammaire riches par segment de mot | Quran | **On pense :** pas encore entièrement exporté |
| **Graphes dépendance / treebank** | Graphes syntaxiques style Iʿrāb par ayah | Quran | **On ne sait pas** — outillage, pas JSON catalogue |
...
**Important maintenant :** `/resources/morphology` n'est **pas** l'outil contributeur mutashabihat (`/morphology_phrases`). Même domaine d'équipe, produits différents.
...
**On sait :** Tutoriel officiel : `app/views/docs/markdown/tutorial-morphology-end-to-end.md`
...
| `sequence_number` | unique global | **On sait :** index unique sur `words` ; ordre interne |

**On pense :** Toujours joindre la morphologie aux données mot Quran Script sur `location`, pas sur le texte arabe seul (homographes existent).
...
|---|---|---|---|
| `root` | mot | `roots`, `root_words` | **On sait :** implémenté |
| `lemma` | mot | `lemmas`, `lemma_words` | **On sait :** implémenté |
| `stem` | mot | `stems`, `stem_words` | **On sait :** implémenté |
| `root` | ayah | `roots` (verse_key, text) | **On sait :** via `verse_root` |
| `lemma` | ayah | `lemmas` (verse_key, text) | **On sait :** via `verse_lemma` |
| `stem` | ayah | `stems` (verse_key, text) | **On sait :** via `verse_stem` |
| `morphology` | any | — | **On sait :** `export_morphology` retourne `todo` ("pending.json") |

**Dans le code actuel :** L'export morphologie grammaticale complet **n'est pas implémenté** dans l'exporteur. Seules les variantes root/lemma/stem sont livrées aujourd'hui.
...
**On sait :** Les gloses anglaises Lemma peuvent être importées en masse via `lib/tasks/import_lemmas.rake` depuis CDN `lemmas.txt`.
...
**On sait :** Couleurs tags POS définies dans le modèle (`POS_TAG_COLORS`) pour rendu UI.

**On pense :** Ce sont les données qu'on voudrait dans un export `morphology` complet. Actuellement non exposées via `ExportQuranicMorphology`.
...
Utilisé par UIs treebank/graphe et validateurs. **On sait :** testé dans `test/lib/corpus/morphology_types_test.rb`.
...
**On sait :** Enum `review_status` : `draft`, `approved`, `need_correction`.
...
**On sait :** Labels relation arête internationalisés via `I18n.t("morphology.edge_relations.#{label}")`.

**On sait :** Route API existe : `GET /api/morphology/edge_relations` (listée dans `config/routes.rb`).
...
**On pense :** Pipeline mainteneur spécialisé — pas le flux brouillon QuranEnc standard.
...
**On sait :** `Morphology::MatchingVerse` (CMS) stocke ayahs correspondantes suggérées avec scores — utilisé dans UI relecture phrases.
...
**On sait :** Les données script mot et morphologie sont des **ressources téléchargeables séparées** — tu dois joindre côté client.
...
3. **`export_morphology` est un stub** — ne t'attends pas à un SQLite POS/grammaire complet dans `/resources` pour l'instant.

4. **`Morphology::Phrase` est CMS** — ne le requête pas via connexions `QuranApiRecord`.
...
6. **Les colonnes texte arabe se multiplient** — la morphologie utilise `text_qpc_hafs` ; associe toujours la ressource script avec laquelle tu pair.
...
| # | Question | Label |
|---|---|---|
| 1 | Calendrier/plan pour export SQLite `morphology` complet (données WordSegment) | **On ne sait pas** |
| 2 | Si les graphes de dépendance deviendront un type de ressource téléchargeable | **On ne sait pas** |
| 3 | Quelles tables morphologie existent seulement dans le dump prod complet vs mini dump | **On ne sait pas** jusqu'à la Phase 17 |
| 4 | Relation entre `Morphology::Word` et `Morphology::WordSegment` dans l'UI | **On pense** — Word est la ligne parent ; segments sont les morceaux sub-mot |
...
En tant que contributeur : sache quelle couche tu touches avant de choisir un dossier.
...
Réponds avec tes questions, ou dis **« proceed »** pour la **Phase 13 — Layouts mushaf**.
