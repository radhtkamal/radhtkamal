...
> **Ce fichier :** remonter ce que tu vois dans le navigateur vers les routes, contrôleurs, vues, presenters et modèles.

---

## Comment tracer n'importe quelle URL (méthode)

Chaque requête navigateur suit la même chaîne :
...
**Raccourci logs :** En développement, chaque requête log `Processing by SomeController#action`.  
Commence là quand l'URL est ambiguë.

---

## Héritage des contrôleurs (qui exécute ta requête)
...
**On sait :** `CommunityController` définit `@presenter = CommunityPresenter`.  
Il fournit `load_language`, `authorize_access!`, et les helpers docs.  
La plupart des outils contributeur en héritent.

**On sait :** `ApplicationController#init_presenter` s'exécute sur chaque requête HTML sauf si un enfant le surcharge.
...
**On pense :** Les changements d'onglet sont des **requêtes HTTP séparées** vers Turbo Frames.  
Pas un état d'onglet côté client.
...
Vue : `app/views/resources/detail.html.erb` → rend un partial de prévisualisation sous `app/views/resources/previews/`.
...
Les octets d'export ont été construits par `refresh_export!` (Phase 11).  
Pas générés sur cette requête.
...
**Piège :** `:id` dans l'URL est **`verses.id`** (entier), pas `verse_key`.  
Le paramètre de requête `resource_id` sélectionne quel package de traduction.
...
**On sait :** La relecture contributeur crée des **lignes de brouillon** (Phase 9).  
Le job d'approbation CMS promeut vers `translations`.
...
C'est le **hub éditorial** (Phase 8).  
Le navigateur affiche du HTML Active Admin ; les actions mettent des jobs Sidekiq en file.
...
**On pense :** Tout `ResourceContent` n'a pas un `DownloadableResource`.  
Vérifie le scope `with_downloadable_resources` dans l'admin.
...
**Signal d'obsolescence :** `contributing.md` dit encore d'éditer `docs/` — **faux**.  
Édite `app/views/docs/markdown/` + `config/docs.yml`.
...
`ToolsHelper#developer_tools` retourne un tableau codé en dur d'objets `ToolCard` avec URL.  
Ça mappe 1:1 aux contrôleurs contributeur listés en Phase 7.

**On pense :** Ajouter un nouvel outil public nécessite : contrôleur + routes + une entrée `ToolCard` dans `tools_helper.rb`.
...
Quand une page 404, vérifie **quel espace de noms d'id** la route attend.
...
**En débogage :** Si les données semblent fausses dans le navigateur mais le contrôleur est mince, lis le presenter.
...
Les pages navigateur rendent surtout du HTML.  
Les clients mobile/JS peuvent toucher du JSON parallèle :
...
**On sait :** Toute fonctionnalité navigateur n'a pas un équivalent API.  
Les téléchargements restent le chemin d'intégration stable.
...
**On sait :** `config/routes.rb` définit `/segment_pipeline/*` → `SegmentPipeline::RunsController`.  
Mais **aucun fichier contrôleur** n'existe dans ce dépôt (signal Phase 16).

Si tu visites ces URL en local, attends des erreurs de routage/chargement.
...
Apprends les **espaces de noms des paramètres** (`verse_key` vs `verse_id` vs `resource_content_id`).  
Et le **triangle CMS ↔ catalogue ↔ outil** — ça résout la plupart des questions « où est ce code ? ».

---

## Arrêt ici — questions avant la Phase 19

La Phase 19 propose **une expérience d'apprentissage contrôlée** (un petit changement sûr que tu peux faire en local).

1. Pour `/translation_proofreadings/1234?resource_id=131`, à quoi `1234` fait référence vs `131` ?
2. Quel fichier ouvrirais-tu en premier pour tracer `/ayah/2:255` ?
3. Où vit réellement sur disque le contenu de `/docs/getting-started` ?

Réponds avec tes questions, ou dis **« proceed »** pour la **Phase 19 — Une expérience d'apprentissage contrôlée**.
