...
> **Ce fichier :** comment tout le système s'articule — couches, frontières et chemins des données de l'édition au téléchargement.

---

## Le modèle en une phrase

QUL est un **monolithe Rails** qui **organise le contenu Coran dans PostgreSQL**, **publie des fichiers d'export versionnés vers object storage**, et **sert un catalogue public + outils contributeurs + API JSON partielle** — avec travail lourd différé à **Sidekiq** et logique longue dans **`lib/`**, pas des controllers minces.

Si tu ne retenais qu'une chose : **éditer dans base Coran → cataloguer dans base CMS → exporter vers S3/CDN → consommateur télécharge JSON/SQLite.**
...
**On pense :** Production s'exécute comme **image Docker** déployée sur **Kubernetes** au push sur `main` (`.github/workflows/deploy.yml`). Dev local utilise `bin/dev` (Rails + esbuild + Tailwind via Foreman).

---

## Carte des couches — responsabilités, E/S, état, confiance

Chaque ligne est une **frontière** que tu traverseras souvent en tant que contributeur.

| Couche | Responsabilité | Entrées principales | Sorties principales | Stateful ? | Frontière de confiance |
|---|---|---|---|---|---|
| **Site web public** | Marketing, catalogue ressources, docs, recherche | HTTP, `DownloadableResource.published` | HTML (ERB + Turbo), redirections vers fichiers | Session/cookies via Devise | **Lecture non fiable** — catalogue publié + docs uniquement |
...
| **`lib/exporter`** | Sérialiser lignes Coran → JSON/SQLite/ZIP, attacher à `DownloadableFile` | Requêtes base Coran, `ResourceContent` | Fichiers dans `tmp/export`, S3 via Active Storage | Exports publiés = **snapshots immuables** | Exports publiés = **surface confiance publique** |
...
| **Active Storage + S3** | Enregistrer zips téléchargeables, assets segments, backups | Fichiers temp locaux depuis exporteur | Objets S3 publics ou privés | `DownloadableFile.token` pour indirection | Dev utilise `:local` ; prod `:qul_exports` |
...
| **`api/v1`** | Accès machine-readable (chapters, translations, tafsir, métadonnées audio) | Clés API / endpoints publics | JSON | **On pense :** plus léger que catalogue download ; docs disent encore API « coming soon » pour surface complète | Rate/stats via `ApiClient` + `UpdateApiStatsJob` |
...
Ce sont les chemins que tu devrais pouvoir dessiner de mémoire.
...
**On sait** — `ResourcesController#download` recherche `DownloadableFile` par token, trace le téléchargement, puis redirige vers `file.file.url` (Active Storage).

**On sait** — `DownloadableFile` utilise `service: :local` en développement, `:qul_exports` sinon (`app/models/downloadable_file.rb`).

**On sait** — `DownloadableResource#refresh_export!` dispatch vers la bonne méthode `Exporter::DownloadableResources` selon `resource_type` et `cardinality_type` (ayah vs word vs chapter).
...
**On sait** — Relecture communautaire (`translation_proofreadings`, `tafsir_proofreadings`) écrit des lignes **`Draft::*` dans le CMS**, pas le texte Coran publié directement (Phase 3).

**On sait** — Actions admin `ResourceContent` enqueue `DraftContent::ApproveDraft*Job` et `DraftContent::ImportDraftContentJob` (`app/admin/content/resource_content.rb`).

**On sait** — `DraftContent::CheckContentChangesJob` planifié hebdomadaire (dimanche 06:00) selon `config/sidekiq_scheduler.yml` — vérifie sources traduction/tafsir externes.
...
**On pense :** Ce chemin est comment la plupart des **contributeurs communautaires** vivent QUL — pas Active Admin.
...
| **SQLite Segments** | `Segments::Base` | Sessions édition segments par récitateur | **On pense :** éphémère/outillage ; exporté retour Coran/S3 |
...
**On sait** — `QuranApiRecord::CDN_HOST = 'https://static-cdn.tarteel.ai'` pour images mot/ayah.

**On sait** — URL dump données Coran dev : `https://static-cdn.tarteel.ai/qul/mini-dumps/mini_quran_dev.sql.zip`.

---

## Réalité frontend dans le monolithe

QUL n'est **pas** une SPA. Pense **app Rails multi-surface** :
...
**On sait** — `bin/dev` exécute trois processus : serveur Rails, `yarn build --reload` (esbuild), `tailwindcss:watch`.

**On pense :** En tant qu'ingénieur React, tu te sentiras le plus à l'aise dans `segments/` et `svg/` — partout ailleurs c'est server-rendered avec des touches de JS.

---

## Travail en arrière-plan — ce que fait Sidekiq

**On sait** — `config/initializers/sidekiq.rb` définit l'adaptateur ActiveJob sur Sidekiq globalement ; Sidekiq Web UI monté sur `/sidekiq` pour admins.
...
**On sait** — `AsyncResourceActionJob` est un dispatcher générique : `perform(resource, :refresh_export!, ...)` — pattern utile en traçant boutons admin.

**On sait** — Cache production utilise Memcached (`config.cache_store = :mem_cache_store`), pas Redis — Redis est pour jobs.

---

## Surface API vs catalogue téléchargement

Deux « APIs » différentes existent ; QUL brouille la ligne.

| Mécanisme | Statut | Meilleur pour |
|---|---|---|
| **Téléchargements `/resources`** | **On sait** — principal, documenté, stable | JSON/SQLite bulk pour apps |
| **`/api/v1/*`** | **On sait** — routes existent (chapters, translations, tafsir, métadonnées audio) | Requêtes live, payloads plus petits |
...
**On pense :** Pour contribution OSS, **exports sont le contrat** sur lequel les consommateurs comptent ; `api/v1` est une surface secondaire en évolution.
...
**On sait** — depuis `.github/workflows/deploy.yml`.

**On sait** — Variables env DB production séparées : `CMS_DB_*` vs `QURAN_API_DB_*` (Phase 5).

**On ne sait pas** — topologie exacte pods (réplica unique vs workers séparés) — non définie dans le workflow de ce dépôt ; **on pense :** web + Sidekiq probablement conteneurs séparés dans K8s.

---

## Cinq faits d'architecture à retenir

1. **Deux bases, une app.** État workflow CMS et contenu canonique Coran sont séparés. Refs cross-DB sont IDs logiques, pas clés étrangères. (Phase 5)
...
| **17 — Setup local** | Rendre ce diagramme exécutable sur ta machine |
...
| # | Question | Label |
|---|---|---|
| 1 | Web et Sidekiq sont-ils déploiements K8s séparés ? | **On ne sait pas** — pas dans workflow dépôt |
| 2 | Modèle auth complet `api/v1` et rate limits | **On ne sait pas** — lire `ApiClient` + controllers phase ultérieure |
| 3 | Cycle de vie SQLite Segments — quand créé/détruit ? | **On ne sait pas** — plongée Phase 14/segments |
| 4 | Connexion DB modèle `Feedback` vs table CMS `feedbacks` | **On ne sait pas** — depuis Phase 5 |
| 5 | Tous exports production passent-ils par Active Storage ou certains contournent via `UploadToCdn` directement | **On pense :** les deux chemins existent ; répartition exacte par resource_type TBD Phase 11 |

---

## Résumé Phase 6

Tu as maintenant une **carte en couches** : navigateurs frappent surfaces Rails (public, contributeur, CMS, API) ; Rails lit/écrit deux bases PostgreSQL ; Sidekiq exécute pipelines `lib/` ; exports atterrissent S3/CDN ; consommateurs téléchargent fichiers versionnés.
...
## Arrête-toi ici — questions avant Phase 7

Avant la visite du dépôt, il aide de vérifier ton modèle mental :

1. Peux-tu expliquer pourquoi `DownloadableResource` vit dans la base CMS mais pointe vers `resource_content_id` dans la base Coran ?
2. Quel chemin tracerais-tu pour trouver pourquoi un JSON traduction sur `/resources` est obsolète ?
3. Où regarderais-tu d'abord pour logique import bulk vs handler bouton admin ponctuel ?

Réponds avec des questions, ou dis **« proceed »** pour **Phase 7 — Visite du dépôt** (carte dossier par dossier liée à ces couches).
