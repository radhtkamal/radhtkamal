...
> **Avant :** [Phases 1–21](phase-01-what-is-qul.md)  
...
| **origin** | Ton fork sur GitHub | Où tu pousse les branches |

**On sait :** La branche par défaut est **`main`**. CI (CodeQL) et workflows deploy se déclenchent sur `main`.

**On pense :** Les PR ciblent `main`, pas `master` ni des branches develop de longue durée.
...
Clique sur **Fork** sur https://github.com/TarteelAI/quranic-universal-library
...
**On sait :** Documenté dans `app/views/docs/markdown/contributing.md`.

### 4. Environnement local

Suis [Phase 17](phase-17-local-setup.md) : `bin/setup`, charger le mini dump, `bin/dev`.
...
**Ligne directrice :** Ne jamais construire une branche feature sur un `main` obsolète. QUL évolue vite.
...
Les commits upstream récents mélangent les styles (`fix:`, `docs:`, impératif simple). **Choisis un et reste cohérent dans ta PR**.
...
- Le numéro PR `(#745)` est ajouté **par le mainteneur au merge** — ne le simule pas dans les commits locaux
...
**UI GitHub :** Compare `TarteelAI/quranic-universal-library` `main` ← `YOUR-USERNAME/quranic-universal-library` `fix/segment-validator-overlap`
...
**On sait :** Le template PR vit dans `.github/PULL_REQUEST_TEMPLATE/pull_request_template.md`.

---

## Garder ton fork à jour (pendant la revue)

Pendant que ta PR est ouverte, `main` upstream peut avancer. Mets à jour ta branche :
...
**Quand rebaser :** Le mainteneur le demande, ou tu veux une PR propre à un seul commit.  
**Quand merger :** Tu n'es pas à l'aise avec force-push, ou la PR a plusieurs commits logiques à préserver.
...
| `onboarding/` | Si les mainteneurs veulent cette série upstream (demande d'abord) |
...
**On sait :** `.env.sample` documente les vars S3 avec valeurs vides par défaut. Copier vers `.env` en local, ne jamais commiter `.env`.

### Fichiers d'intégration dans ton clone

La série `onboarding/phase-*.md` peut être **non trackée** dans ton working tree. Avant de pousser :

```bash
git status
```

Décide : inclure dans une PR `docs/onboarding`, garder local uniquement, ou publier séparément. Ne mélange pas accidentellement 24 fichiers d'intégration dans une PR code sans lien.
...
### Inspecter ton diff avant d'ouvrir
...
```bash
git log upstream/main..HEAD --oneline    # commits uniquement sur ta branche
git diff --name-only upstream/main       # fichiers changés vs upstream
gh pr status                              # état PR si tu utilise gh CLI
...
git fetch upstream
git merge upstream/main
git push origin main          # sync main de ton fork

git branch -d fix/my-branch   # supprimer branche locale
```

**On pense :** Upstream utilise probablement le **squash merge** (un commit par PR avec `(#NNN)` dans le titre). Les hash de commits de ta branche n'apparaîtront pas sur `main`. C'est normal.

---

## Gérer les retours de revue

| Le reviewer demande | Tu fais |
|---|---|
...
Réponds sur la PR avec ce que tu as changé. Re-exécute les tests que tu cites.
...
```text
         ...
         ┌──────────────┴──────────────────────────────┐
         │  YOUR-USERNAME/quranic-universal-library      │  ← origin (cible de tes push)
...
Tu n'as **jamais** besoin d'accès en écriture à `TarteelAI/*`. Fork + PR est tout le modèle.
...
**On sait :** Le template PR encourage de lier les issues ouvertes.

---

## CI sur ta PR

**On sait :** `.github/workflows/codeql.yml` s'exécute sur les PR vers `main`.

**On sait :** Pas de workflow automatisé `rails test` ou `rubocop` (Phase 20).

**On pense :** CodeQL vert ≠ barrière qualité complète. Exécute les tests en local et dis-le dans la PR.
...
| # | Question | Label |
|---|---|---|
| 1 | Stratégie squash vs merge commit | **On pense :** squash d'après les numéros PR dans les titres de commit |
| 2 | Commits signés requis | **On ne sait pas :** pas documenté |
| 3 | Règles protection branche sur `main` | **On pense :** PRs requises ; règles exactes dans paramètres GitHub |
| 4 | Si `onboarding/` sera accepté upstream | **On ne sait pas :** coordonner avec les mainteneurs |
...
Les mécaniques git sont OSS standard. Les pièges spécifiques QUL sont **chemin docs**, **ne pas commiter dumps**, et **garder les changements données hors des PR code**.

---

## Arrête-toi ici — avant la Phase 23

La Phase 23 est la **carte des surfaces de contribution + évaluation de préparation**. Où tu peux contribuer maintenant, et une liste de contrôle honnête.

1. Quel remote `git push` doit cibler — `origin` ou `upstream` ?
2. Dans quelle branche les PR mergent ?
3. Que dois-tu exécuter avant d'ouvrir une PR si la CI ne lance pas les tests ?

Réponds avec des questions, ou dis **"proceed"** pour la **Phase 23 — Carte des surfaces de contribution**.
