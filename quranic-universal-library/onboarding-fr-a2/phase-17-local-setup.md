...
> **Ce fichier :** faire tourner QUL sur ta machine, charger les données Quran, et vérifier que tout fonctionne.
...
| **Mini dump Quran** | Toute page avec versets/traductions/audio |
| **Redis** | Jobs `perform_later`, Sidekiq (optionnel pour une première visite) |

**On sait :** Doc officielle : `app/views/docs/markdown/project-setup.md` (servie sur `/docs/project-setup`).

**On pense :** Tu peux démarrer l'app et parcourir `/docs` sans le dump.  
Mais `/ayah/2:255`, les prévisualisations `/resources` et la plupart des outils échoueront tant que `quran_dev` n'est pas rempli.
...
**On sait :** Le Gemfile autorise `ruby ">= 3.3.3", "<= 3.3.10"`.  
Reste sur 3.3.3 sauf raison documentée de monter.
...
**On sait :** `db/schema.rb` est **CMS uniquement**.  
Le schéma Quran vient du dump SQL, pas des migrations.
...
### 2. Variables d'environnement pour la base de données
...
**On pense :** Avec les trois non définies, Rails utilise les défauts PostgreSQL.  
Souvent auth peer sur macOS/Linux avec socket local.  
Si la connexion échoue, définis explicitement :
...
Ou crée `.env` depuis `.env.sample` (vars S3 seulement dans l'exemple — ajoute les vars DB toi-même).

**On sait :** Le gem `dotenv` est dans le groupe `:development`.  
Il n'y a pas de `Dotenv.load` évident dans `config/`.  
**On pense :** exporte les vars dans ton shell ou utilise un outil qui charge `.env` avant `bin/dev`.
...
**On pense :** Si `\dt quran.*` ne montre aucune table, le dump n'a pas été chargé dans la bonne base.

### 4. Seed utilisateur admin CMS (optionnel mais utile)

`bin/setup` n'exécute **pas** les seeds.  
Crée le super-admin par défaut manuellement :

```bash
bin/rails db:seed
```

**On sait :** `db/seed.rb` crée :
...
Utilise ceci pour `/cms` et `/sidekiq` (admin uniquement).

Tu peux aussi enregistrer un utilisateur normal via Devise sur `/users/sign_up`.  
Un workflow d'approbation peut s'appliquer pour les rôles non-admin.
...
### 6. Redis + Sidekiq (quand tu as besoin des jobs async)
...
Sans Sidekiq, les jobs `perform_later` s'accumulent dans Redis mais ne s'exécutent jamais.  
Les chemins `perform_now` (sauvegarde mushaf, approbation brouillon unique) fonctionnent toujours.
...
Puis définis l'env et lance Rails sur l'hôte :
...
**On sait :** `docker/README.md` documente un build d'image **production** (`docker build`).  
Pas une stack compose dev tout-en-un.
...
| `RAILS_MASTER_KEY` | Image Docker production | **Non requis** pour dev |
| `REDIS_URL` | Action Cable prod | dev cable.yml défaut `redis://localhost:6379/1` |

**On pense :** Tu peux développer en local **sans** credentials S3 réels.  
Les actions admin export-vers-S3 échoueront tant que les credentials ne sont pas configurés.

---

## Liste de vérification

Exécute ceci après la configuration.  
C'est un smoke test, pas la suite de tests complète.
...
| `CMS users: 0` | Exécute `bin/rails db:seed` si tu as besoin de connexion admin |
...
Le processus js de `bin/dev` devrait logger `[reload] listing on port 3005`.  
Éditer un fichier Stimulus devrait déclencher un rafraîchissement navigateur.
...
Si tu corromps les données Quran locales :
...
**AVERTISSEMENT** — `ExportMiniDumpJob` supprime de larges portions de `quran_dev` avant `pg_dump`.  
Ne jamais l'exécuter hors développement.

---

## Signaux de documentation obsolète

| La doc dit | Dans le code actuel | Label |
|---|---|---|
| `contributing.md` : « Éditez les fichiers dans `docs/` d'abord » | Les docs vivent dans `app/views/docs/markdown/` + `config/docs.yml` | **On sait** — chemin obsolète |
| `.github/copilot-instructions.md` : admin sur `/admin` | Les routes redirigent vers `/cms` | **On sait** — URL obsolète |
| `copilot-instructions.md` : `rails db:create` seulement | `bin/setup` utilise `db:create:all` pour les deux bases | **On sait** — incomplet |
| `project-setup.md` implique que le dump est optionnel | L'app démarre mais les pages Quran échouent | **On pense** — la doc sous-estime l'importance du dump |

Préfère **`app/views/docs/markdown/project-setup.md`** et cette série d'intégration à `contributing.md` à la racine.

---

## Notes rapides macOS

**On pense :** Combinaison courante qui fonctionne :
...
| # | Question | Label |
|---|---|---|
| 1 | Si `dotenv` charge auto `.env` sans config supplémentaire | **On ne sait pas** — gem présent, aucun loader explicite |
| 2 | Chemin dev officiel : Postgres natif vs Docker | **On pense** — `bin/setup` + Rails sur l'hôte est principal |
| 3 | Si `config/master.key` est obtenable pour les contributeurs | **On ne sait pas** — pas dans le dépôt |
| 4 | Fraîcheur / cadence de mise à jour du mini dump | **On ne sait pas** — l'URL CDN est un chemin statique |
...
La configuration locale est une **base de données en deux étapes** : migrations pour CMS, dump pour Quran.  
Tout le reste est Rails 8 + esbuild standard.

---

## Arrêt ici — questions avant la Phase 18

La Phase 18 corrèle **les URL du navigateur au code** — mapper ce que tu vois sur `/ayah/2:255` ou `/translation_proofreadings` aux contrôleurs, vues et modèles.

1. Quelle base de données contient `verses`, et comment la peupler en local ?
2. Que ne fait pas `bin/setup` que tu dois encore faire manuellement ?
3. Quelle URL utilisez-vous pour le CMS, et quelles sont les credentials seed par défaut ?

Réponds avec tes questions, ou dis **« proceed »** pour la **Phase 18 — Corrélation navigateur ↔ code**.
