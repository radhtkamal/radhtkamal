...
> **Ce fichier :** ce qu'est réellement le frontend QUL — pas une SPA React, mais Rails + Stimulus, Turbo, jQuery et deux îlots Vue.

---

## La vérité en une phrase

QUL n'est **pas** une application frontend unique.  
C'est un **site rendu par serveur Rails** avec amélioration progressive :
...
**On pense :** Si tu viens de React/Next.js, attends-toi à **pas d'arbre de composants**, **pas de store global**, et **pas de routeur côté client**.  
La plupart des pages sont des allers-retours HTML complets ou des échanges Turbo Frame.

La Phase 4 a brièvement introduit cette pile.  
Cette phase approfondit **où vit le code**, **comment les pages bootent le JS**, et **quel schéma utiliser pour du nouveau travail**.
...
**On sait :** Il n'y a **pas de Webpacker, Vite ou importmap** dans ce dépôt.  
Les scripts `package.json` utilisent `esbuild` + `esbuild-rails` + `esbuild-plugin-vue3`.
...
**On sait :** `Procfile.dev` ne démarre pas Redis ni Sidekiq (Phase 15).

`yarn build --reload` injecte un listener EventSource.  
Il recharge le navigateur quand JS, Vue, ERB ou CSS change.
...
**On sait :** Le CMS enregistre Tailwind + Font Awesome dans `config/initializers/active_admin.rb`.  
Il n'appelle **pas** `register_javascript` — le layout AA charge `active_admin.js` via Sprockets.
...
Vue lit la config depuis les **attributs `data-*`** sur l'élément parent.  
Pas depuis un endpoint bootstrap JSON API.
...
`esbuild-rails` importe en glob chaque `*_controller.js`.  
La convention mappe chemin de fichier → nom de contrôleur :
...
**On pense :** Le nouveau code devrait préférer le DOM vanilla ou les API Stimulus.  
Mais suivre le style jQuery existant dans un fichier est acceptable pour la cohérence.
...
Ce sont des **mini-apps dans un contrôleur** — lis attentivement avant de modifier.
...
L'éditeur de mise en page mushaf utilise des frames pour la prévisualisation :
...
**Schéma pont :** Rails passe l'état serveur via `data-*` sur `#app`.  
Vue `mounted()` lit `this.$el.parentElement.dataset` et commit dans Vuex.
...
**On sait :** Pas de Vue Router. Une page, un montage, pas de routes côté client.
...
**On pense :** jQuery est **legacy mais actif**. Ne le retire pas à la légère — beaucoup d'outils en dépendent.
...
Les endpoints JSON renvoient du JSON rendu par Rails ou des payloads Jbuilder.  
Le token CSRF est lu depuis `<meta name="csrf-token">`.
...
|---|---|
| `tailwind.css` | Classes utilitaires (layout principal) |
...
Les outils contributeur mélangent **classes Tailwind dans l'ERB** avec **classes SCSS legacy** (ex. `.mushaf`, `.qpc-hafs`).

Polices arabes chargées via `@font-face` dans `app/assets/stylesheets/shared/`.  
Critiques pour le rendu mushaf et tajweed.
...
**On sait :** `package.json` inclut **Cypress** et **Puppeteer** comme devDependencies.

**On pense :** Des tests E2E existent mais ne sont pas le workflow contributeur principal.  
Linting : ESLint sur `app/javascript/**/*.{js,vue}`.
...
| Nouvel outil avec état complexe + UI timeline | **Îlot Vue** (nouveau point d'entrée esbuild) — rare ; discute d'abord |
| Comportement de champ CMS | JS **Active Admin** ou Stimulus dans `for_admin` |
| Page catalogue public | **ERB + Tailwind** |

**Réponse par défaut :** Stimulus + ERB.  
Vue seulement quand le constructeur de segments prouve le schéma (grand état interactif, UI forme d'onde audio).
...
| Signal | Label |
|---|---|
| Routes `segment_pipeline` dans `config/routes.rb` mais **aucun** contrôleur `SegmentPipeline::*` | **On sait** — routes peuvent 404 en local |
| `segment_tool.js` dans `assets.precompile` mais **aucun fichier source** | **On sait** — référence asset obsolète |
| `segments/index.css` / `svg/index.css` référencés dans les vues mais absents de `app/assets/stylesheets/` | **On pense** — générés dans `app/assets/builds/` par esbuild |
| `Importer::TafsirApp.delay` commenté (syntaxe Delayed Job) | **On sait** — legacy mort |
...
3. **Turbo peut casser les widgets jQuery** — utilise `data-turbo="false"` ou des listeners `turbo:load`.

4. **Vue n'est pas partagé** — constructeur de segments et optimiseur SVG ne s'importent pas mutuellement.

5. **`$` global** — jQuery est sur la plupart des pages ; évite les collisions de noms.
...
7. **Deux systèmes CSS** — utilitaires Tailwind + composants SCSS coexistent ; vérifie les deux pour le style.
...
| # | Question | Label |
|---|---|---|
| 1 | Si l'UI `segment_pipeline` vit dans un gem privé ou une branche inachevée | **On ne sait pas** — routes + schéma existent, contrôleurs absents |
| 2 | Pipeline d'assets production (CDN vs builds auto-hébergés) | **On pense** — précompilation Sprockets standard au déploiement |
| 3 | Étendue de la couverture tests Cypress | **On ne sait pas** — dépendance présente, non inventoriée |
| 4 | Plan de migrer plus d'outils vers Vue vs garder Stimulus | **On ne sait pas** — préférence organisationnelle |
...
Le frontend est **volontairement sobre** sur la plupart des pages.  
La complexité est localisée aux segments audio et au rendu de graphes morphologiques.

---

## Arrêt ici — questions avant la Phase 17

La Phase 17 couvre la **configuration locale** — exécuter `bin/setup`, charger le dump Quran, et vérifier la pile.
...
Réponds avec tes questions, ou dis **« proceed »** pour la **Phase 17 — Configuration locale**.
