...
> **Ce fichier :** comment le contenu Quran publié devient les fichiers JSON/SQLite que les consommateurs téléchargent depuis `/resources`.

---

## Principe central

**FAIT** — Le produit public est des **artefacts d'export versionnés**, pas l'accès base de données live. L'export lit les lignes **publiées** depuis la DB Quran, les sérialise en fichiers, les zippe, les upload sur S3 via Active Storage, et les enregistre sur `DownloadableResource` dans la DB CMS.
...
**INFÉRENCE :** En tant que contributeur corrigeant des données orientées consommateur, vous vous souciez de **`refresh_export!`** — pas des jobs export sidebar.
...
**FAIT** — `export_all` est documenté en haut du fichier pour usage console. **INFÉRENCE :** Les rafraîchissements production sont par ressource via Sidekiq, pas `export_all` nocturne.
...
**FAIT** — Les clés sont des chaînes `verse_key` (`"surah:ayah"`), indexées depuis `Translation.verse_key`.
...
**FAIT** — Les noms de colonnes utilisent `sura`/`ayah` dans les exports SQLite (la doc consommateur référence `surah_id`/`ayah_number` — mêmes entiers, noms différents).
...
**FAIT** — `config/storage.yml` `qul_exports` utilise les variables env `QUL_STORAGE_*` avec `public: true` et cache control 1 an.
...
**INFÉRENCE :** Une entrée tafsir peut couvrir plusieurs ayahs ; l'export duplique la référence de groupe par clé verset pour commodité de lookup.
...
**FAIT** — Seuls les utilisateurs qui ont précédemment téléchargé **un fichier** sur cette ressource reçoivent un email.
...
**INFÉRENCE :** Ces chemins supportent Tarteel mobile / outillage interne — pas le catalogue zip `/resources`.
...
**INFÉRENCE :** Préserve les entités HTML et Unicode dans le texte de traduction sans sur-échappement — important pour le balisage arabe/HTML notes de bas de page dans les consommateurs JSON.
...
**INFÉRENCE :** `export_all` commence par `FileUtils.rmdir("tmp/export")` — l'export complet efface d'abord le répertoire temp.
...
**FAIT** — L'approbation brouillons (Phase 8) **ne déclenche pas** automatiquement l'étape 3.
...
| # | Question | Label |
|---|---|---|
| 1 | `export_all` est-il exécuté selon un planning en production ? | **INCONNU** — pas dans le scheduler du dépôt ; rafraîchissement par ressource uniquement |
| 2 | Upload CDN direct (`UploadToCdn`) vs Active Storage pour traductions | **INFÉRENCE** — le catalogue utilise Active Storage ; `UploadToCdn` utilisé pour segments audio/manifestes |
| 3 | Si le flag `published` sur `DownloadableResource` bascule auto à l'export | **INFÉRENCE** — `run_export_action` définit `published` à true si nil |
| 4 | Stabilité exacte URL S3 entre rafraîchissements | **INFÉRENCE** — nouvelle clé objet à chaque rafraîchissement (timestamp dans clé) ; URL token sur QUL reste stable |
...
La couche export est le **contrat public**. Si le JSON est faux, tracez : contenu DB Quran → classe exporteur → variante `file_type` → pas les tables brouillon import.
...
Répondez avec vos questions, ou dites **« proceed »** pour la **Phase 12 — Morphologie / données linguistiques arabes**.

---

*Généré pendant l'onboarding contributeur QUL. Phase 11 sur ~24. Investigation en lecture seule — aucun code modifié.*
