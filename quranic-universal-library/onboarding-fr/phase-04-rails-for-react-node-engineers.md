...
> **Ce fichier :** vocabulaire Rails minimum pour lire QUL — seulement les concepts réellement présents dans ce codebase.

---

## Ce que vous apprenez

Vous n'avez pas besoin de devenir expert Rails. Vous avez besoin de vocabulaire suffisant pour :
...
Cette phase mappe les pièces Rails inconnues vers des choses que vous connaissez déjà depuis Express, React et Firebase.
...
| Turbo | `@hotwired/turbo-rails` | Mises à jour partielles de page sans rechargement complet |
...
**FAIT :** Tous les points d'entrée HTTP sont déclarés dans un fichier. QUL a ~260 lignes de routes couvrant :
...
**Convention Rails :** `resources :translation_proofreadings` génère des routes RESTful (`index`, `show`, `edit`, `update`, …). Vous verrez à la fois `resources` RESTful et routes explicites `get`/`post`.
...
**FAIT :** Les anciennes URLs `/admin` redirigent vers `/cms` (301).
...
**FAIT** — comportements clés depuis `app/controllers/application_controller.rb` :
...
**Comparaison Node :** `ApplicationController` est votre middleware Express global + classe router de base. `before_action` est un middleware exécuté avant la méthode action. `protect_from_forgery` est protection CSRF — les formulaires incluent un token caché.
...
**FAIT :** Beaucoup d'actions ayah rendent des **partials** avec `layout: false` — conçus pour injection Turbo Frame (swap d'un panneau sans recharger la page).

### Controllers API

**FAIT :** `Api::V1::ApiController` hérite de `ActionController::API` (pas de CSRF, pas de views) :
...
**Comparaison Node :** C'est un router Express JSON plain — pas de HTML, pas de CSRF, retourne `render json:`.

---

## Presenters (`app/presenters/`) — pattern spécifique QUL

**FAIT :** QUL utilise **44 classes presenter**. Ce n'est pas Rails universel — c'est une convention QUL.
...
**Comparaison Node :** Pensez à un presenter comme un **view model par requête** — comme les résultats d'un hook de fetch de données d'un container React, ou un serializer qui connaît aussi `params` et pagination. Il garde les templates ERB simples et les controllers minces.

**Quand vous voyez `@presenter` dans une view**, cherchez dans `app/presenters/` la logique de requête et formatage.

---

## Models et Active Record (`app/models/`)

### Ce qu'est Active Record (au-delà d'une interface TypeScript)
...
| Validations | Validation schéma entrée (sparse dans modèles contenu QUL) |
...
**FAIT :** `Chapter`, `Verse`, `Word`, `Translation` → `QuranApiRecord`  
**FAIT :** `User`, `DownloadableResource`, `Draft::Translation`, `ChangeLog` → `ApplicationRecord`

**Comparaison Node :** Comme avoir deux bases Firestore ou deux clients Prisma — les modèles se connectent silencieusement à différentes bases PostgreSQL. Vous ne pouvez pas supposer que `belongs_to` fonctionne entre bases.

### Concerns (`app/models/concerns/`) — mixins modèle partagés

**FAIT :** 8 concerns, incluant :
...
C'est important pour QUL. La logique est répartie entre deux emplacements :
...
| `app/services/` | ~26 fichiers | **Services ciblés plus récents :** recherche, rendu docs, édition graphe morphologie, validation audio |
...
**FAIT :** Les exporteurs vivent dans `lib/exporter/` (pas `app/services/`).  
**FAIT :** Les importeurs vivent dans `lib/importer/`.  
**FAIT :** Jobs comme `DraftContent::ApproveDraftTranslationJob` orchestrent imports mais délèguent au code model/service.
...
**Comparaison Node :** `lib/` est comme un dossier `src/lib/` ou `packages/core/` de modules Ruby plain. `app/services/` est plus proche de votre couche service dans une app NestJS/Express.
...
**FAIT :** QUL rend HTML côté serveur avec templates ERB (Ruby embedded) :
...
**FAIT :** Beaucoup de views utilisent `turbo_frame_tag` et templates `.turbo_stream.erb` pour mises à jour partielles.

**Comparaison Node :** Comme fetcher un fragment HTML depuis une API et swapper un `div` — sauf que Turbo le gère avec des conventions (`data-turbo-frame`, réponses `turbo_stream`) au lieu d'écrire `fetch` + `innerHTML`.
...
**FAIT :** Active Admin est une gem qui **génère une UI admin CRUD complète** depuis fichiers configuration Ruby. QUL a **113 fichiers d'enregistrement admin**.

**FAIT :** Monté sur `/cms` (namespace `:cms`).

**FAIT :** Utilise CanCanCan pour autorisation (`config.authorization_adapter = ActiveAdmin::CanCanAdapter`).
...
**Comparaison Node :** Imaginez écrire zéro page admin React, et déclarer à la place `admin.resource('Translation', { filters: [...], columns: [...] })` et obtenir une UI CRUD complète. C'est Active Admin. Ce n'est **pas** une app frontend séparée — c'est du HTML server-rendered avec son propre JS jQuery (`app/javascript/active_admin.js`).

**Quand vous éditez du contenu en tant qu'admin**, vous êtes généralement dans Active Admin, pas les outils contributeurs publics.

---

## Authentification : Devise

**FAIT :** `devise_for :users` avec controllers custom pour inscription et sessions.

**FAIT :** Le modèle `User` inclut :
...
**FAIT :** Les utilisateurs ont des rôles via enum : `normal_user`, `admin`, `super_admin`, `moderator`, `contributor`, `audio_annotator`.

---

## Autorisation : CanCanCan

**FAIT :** La classe `Ability` dans `app/models/ability.rb` définit permissions :
...
**FAIT :** L'édition communautaire utilise une **deuxième couche d'accès** — `UserProject` (assignation projet approuvée par `ResourceContent`), vérifiée via `can_manage?(resource)` dans `ApplicationController`.

---

## Jobs en arrière-plan : Sidekiq

**FAIT :** L'adaptateur `ActiveJob` est Sidekiq. Redis supporte la file.

**FAIT :** Sidekiq Web UI sur `/sidekiq` — uniquement pour super_admin et admin.

**FAIT :** Jobs planifiés dans `config/sidekiq_scheduler.yml` :
...
**FAIT :** `db/migrate/` a ~105 migrations — toutes pour la **base CMS**.

**FAIT :** `db/schema.rb` reflète uniquement la base CMS.

**FAIT :** Les tables contenu Coran ne sont **pas** gérées par migrations Rails dans ce dépôt. Elles viennent du dump `mini_quran_dev.sql` (voir `project-setup.md`).
...
**Quand vous ajoutez une feature CMS** (nouveau champ draft, nouvelles métadonnées download) : écrire une migration.  
...
**FAIT :** Les controllers Stimulus s'enregistrent auto depuis `app/javascript/controllers/index.js` :
...
**Comparaison Node :** Stimulus est comme de petits hooks React attachés aux éléments DOM via attributs `data-controller="ayah-jump"` — pas de virtual DOM, pas d'arbre composants build-time. Bon pour toggles, modals, chargements AJAX.

### Vue

**FAIT :** Seulement deux points d'entrée Vue dans `esbuild.config.js` :
...
**Ne supposez pas que Vue est le défaut.** La plupart de QUL est ERB + Stimulus.

### Pipeline assets

**FAIT :** `bin/dev` exécute Foreman avec `Procfile.dev` :
...
**FAIT :** JS bundlé par esbuild → `app/assets/builds/`. CSS par Sass + Tailwind.
...
Quand vous rencontrez une URL ou un problème :
...
Vous n'avez pas besoin d'écrire du Ruby idiomatique encore. Vous devez le lire.
...
6. **Presenters** sont la couche view-model de QUL — vérifiez-les quand `@presenter` apparaît.
...
| Comment Turbo est configuré globalement | **INCONNU** — utilisé par view ; pas de config globale inspectée |
| Si API v1 est considérée stable | **INCONNU** — docs disent « coming soon » mais routes existent (Phase 1) |
...
1. **Route → controller → presenter → model → view** — commencez chaque investigation là.
...
4. **Deux bases de données** via `ApplicationRecord` vs `QuranApiRecord` — vérifiez toujours quelle classe de base.
5. **Frontend multi-génération** — ERB + Turbo + Stimulus d'abord ; Vue seulement dans deux outils.

---

*Généré pendant l'onboarding contributeur QUL. Phase 4 sur ~24. Investigation en lecture seule — aucun code modifié.*
