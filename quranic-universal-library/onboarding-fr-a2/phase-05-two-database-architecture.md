...
QUL est **une application Rails** qui parle à **deux bases PostgreSQL** (plus un troisième store SQLite optionnel pour l'outillage segments).
...
**Important maintenant :** Un modèle Active Record n'est **pas** automatiquement « la base de données ». Dans QUL, la classe de base détermine quelle base PostgreSQL (et parfois quel schéma) tu touches.

---

## Configuration (`config/database.yml`)

**On sait** — quatre connexions nommées :
...
**On sait** — la connexion Coran définit le search path schéma PostgreSQL :
...
**On pense :** Les tables Coran vivent principalement dans le schéma PostgreSQL **`quran`** dans la base `quran_dev` — pas dans `public`. Des références code comme `quran.text`, `quran.image` (vues dans `ExportMiniDumpJob`) confirment l'existence d'objets qualifiés par schéma.

**On sait** — L'init Docker crée les deux bases et le schéma `quran` :
...
La base Coran est créée vide. Tu dois charger le dump séparément (Phase 17).
...
**On sait :** `QuranApiRecord` hérite encore de `ApplicationRecord` (qui hérite `ActiveRecord::Base`) — mais `establish_connection` sur la classe redirige toutes les requêtes pour cette hiérarchie.
...
**On sait :** Toutes les tables dans `db/schema.rb` (~40 tables). Groupées par rôle :

| Catégorie | Tables | Rôle |
|---|---|---|
| **Auth & users** | `users`, `admin_users` | Comptes Devise, rôles |
| **Couche publication** | `downloadable_resources`, `downloadable_files`, `downloadable_resource_tags`, `downloadable_related_resources`, `user_downloads` | Catalogue public `/resources` et fichiers |
...
| **Communication changements** | `change_logs`, `admin_todos`, `proof_read_comments` | Notes mise à jour, todos mainteneur |
| **Morphologie CMS uniquement** | `morphology_phrases`, `morphology_phrase_verses`, `morphology_matching_verses` | Travail phrases mutashabihat (stocke refs `verse_id`) |
...
**On sait :** Migrations CMS dans `db/migrate/` (~105 fichiers). `db:prepare` / `db:migrate` n'affecte que cette base.

### Base contenu Coran (`quran_dev`)

**On sait :** Non représentée dans `db/schema.rb`. Peuplée via dump `mini_quran_dev.sql` en développement.

**On sait :** Modèles utilisant `QuranApiRecord` (~90+ classes modèle). Familles de tables majeures :
...
**On sait :** `DownloadableResource` (CMS) déclare :

```ruby
belongs_to :resource_content, optional: true  # ResourceContent is QuranApiRecord
```

Rails interrogera la connexion Coran quand tu accèdes à `downloadable_resource.resource_content`. Cela fonctionne à runtime mais :
...
**On sait :** L'import copie le contenu et re-synchronise les champs d'identité depuis `Verse` dans la base Coran, puis marque le draft `imported: true` dans la base CMS. Ces deux écritures ne sont **pas** une transaction atomique unique entre bases.

### Pattern 3 : Modèles CMS avec `belongs_to :verse`

**On sait :** `Morphology::Phrase` (CMS) fait :
...
## Ce que tu ne peux pas faire (contraintes)
...
**On sait :** `ActiveRecord::Base.transaction` encapsule **une connexion** (typiquement CMS par défaut).

**On pense :** Si l'import échoue à mi-chemin (draft marqué imported mais traduction non sauvegardée), tu comptes sur l'idempotence du job et le nettoyage manuel — pas une transaction ACID unique.

### 3. Pas de fichier schéma unique pour la base Coran

**On sait :** `db/schema.rb` = CMS uniquement.

**On pense :** Les changements schéma Coran sont gérés hors workflow migration Rails normal pour contributeurs (restore dump, processus DBA production). Ne t'attends pas à ce que `rails db:migrate` mette à jour `verses`.

### 4. Tests simulent surtout les modèles Coran

**On sait :** `test/test_helper.rb` stub `Chapter`, `Verse`, `ResourceContent` avec fausses classes pour tests unitaires — il ne charge pas le dump Coran complet dans `quran_community_cms_test`.

**On pense :** Couverture test pour comportement données Coran limitée. Vérification manuelle compte pour modifications données.

### 5. Table versions PaperTrail est dans le CMS

**On sait :** Table `versions` dans `db/schema.rb` (base CMS), mais modèles versionnés comme `Translation` et `Verse` sont `QuranApiRecord`.

**On ne sait pas :** Comportement exact stockage PaperTrail pour modèles cross-connection — vérifier en Phase 17. Probablement versions écrites dans CMS `versions` avec `item_type`/`item_id` pointant vers IDs lignes Coran.
...
**Utile plus tard** — pas le modèle deux-DB principal, mais tu peux le rencontrer :
...
**On sait :** `Segments::Database` (CMS) stocke un fichier SQLite via Active Storage. Quand chargé, les modèles analyse segments (`Segments::Position`, `Segments::Failure`, etc.) se connectent dynamiquement à ce fichier SQLite.
...
**On sait** — depuis `lib/utils/db_backup.rb` :
...
**On pense** (appuyé par preuves architecture) :

1. **Séparation échelle et cycle de vie** — Le contenu Coran est volumineux, relativement stable, et partagé avec d'autres systèmes Tarteel. L'état CMS (users, drafts, packaging download) change plus vite et est spécifique à l'app.
...
| Si `uloom_contents` devrait être CMS ou Coran long terme | **On ne sait pas** — actuellement CMS avec cross-refs |
| Liste complète tables/vues schéma Coran | **On ne sait pas sans dump chargé** — inspecter `quran_dev` en Phase 17 |
| Si production utilise même nom schéma PostgreSQL `quran` | **On pense oui** — `schema_search_path` défini dans tous environnements |
...
1. **`ApplicationRecord` = base CMS**, **`QuranApiRecord` = base Coran** — vérifie la classe de base de chaque modèle.
...
5. **`bin/setup` crée `quran_dev` vide** — tu dois charger le dump avant que pages Coran fonctionnent.
